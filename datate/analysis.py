...
def test_net():
    ...
    while True:
        ...
        model(x,t)
        ccff = model(x,t)
        kankan = ccff.data[0].tolist()
        categories = np.loadtxt("labels.txt", str, delimiter="\t")
        prediction = zip(kankan,categories)
        prediction.sort(cmp=lambda x,y: cmp(x[0],y[0]),reverse=True)
        for rank,(score,name) in enumerate(prediction[:1],start=1):
          feigege =str('%s  %4.10f' % (name,score * 100))
          print(feigege)
          print(feigege,file = baituole )
          
#          goushi = ' '
#          print>>baituole,(goushi.join(feigege))
            
#          for h in range(len(score*100)):
...
#          if name not in feijigege: 
#            feijigege[name] = 1
#          else:
#            feijigege[name] = feijigege[name] + 1
