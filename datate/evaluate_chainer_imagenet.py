#the for/lamat of test.txt is like that of train.txt used in chainer/examples/imagenet
...
def test_net():
    zhengque = open('gailvshuchu.txt','w')
    while True:
        while data_q.empty():
            time.sleep(0.1)
        inp = data_q.get()
...
#          feigeger = '%s  %4.10f' % (name,score * 100)
#          zhengque.write(str(feigeger)+"\n")
